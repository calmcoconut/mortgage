...
def scenario_create(request: HttpRequest) -> HttpResponse:
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            print("SCENARIO FORM ERRORS:", form.errors)
    else:
        form = ScenarioForm()

    return render(request, "scenario_form.html", {
        "form": form,
        "is_create": True,
        "scenario": None,
    })
...
def scenario_screen_preview(request: HttpRequest) -> HttpResponse:
    ...
    dummy_input = ScenarioInput(
        ...
        state="CA",
        county_fips=None,
        program=program if program in ["conventional", "fha", "va", "usda"] else "conventional",  # type: ignore
        term_months=360,
    ...
    return render(request, "partials/screening_result.html", {
...
def scenario_compare(request: HttpRequest, scenario_id: UUID) -> HttpResponse:
    scenario = get_object_or_404(ScenarioModel.objects.prefetch_related("loan_options"), id=scenario_id)
    ...
    options_data = []
    baseline_result = comparison.option_results[0] if comparison.option_results else None
    ...
    for opt_res in comparison.option_results:
        ...
        if baseline_result and baseline_result.option_id != opt_res.option_id:
            savings_vs_base = baseline_result.financing_cost_at_horizon - opt_res.financing_cost_at_horizon
        ...
        options_data.append({
            ...
            "apr_pct": f"{opt_model.apr * 100:.3f}%" if opt_model and opt_model.apr else None,
            "points_pct": f"{opt_model.points_pct * 100:.2f}%" if opt_model else "",
            "points_dollars": (opt_model.loan_amount * opt_model.points_pct) if opt_model else Decimal("0"),
        })
    ...
    if comparison.recommended_option_id:
        for item in options_data:
            if item["result"].option_id == comparison.recommended_option_id:
                ...
                break
        if recommended_opt and baseline_result and baseline_result.option_id != comparison.recommended_option_id:
            diff = baseline_result.financing_cost_at_horizon - recommended_opt["result"].financing_cost_at_horizon
...
def scenario_projected_costs(request: HttpRequest, scenario_id: UUID) -> HttpResponse:
    ...
    if len(comparison.option_results) >= 2:
        ...
        savings_at_horizon = opt_a.financing_cost_at_horizon - opt_b.financing_cost_at_horizon
    ...
    context = {
        ...
        "horizon_months": horizon_months,
        "horizon_years": horizon_months // 12,
        "discount_rate_display": f"{discount_rate * 100:.1f}" if discount_rate else "4.0",
...
def option_create(request: HttpRequest, scenario_id: UUID) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        form = LoanOptionForm(initial={
            "loan_amount": scenario.loan_amount,
            "term_months": scenario.term_months,
            "source_type": "manual",
        })

    return render(request, "option_form.html", {
        "form": form,
        "scenario": scenario,
        "is_create": True,
    })
...
def option_edit(request: HttpRequest, scenario_id: UUID, option_id: UUID) -> HttpResponse:
    scenario = get_object_or_404(ScenarioModel, id=scenario_id)
...
def option_delete(request: HttpRequest, scenario_id: UUID, option_id: UUID) -> HttpResponse:
    scenario = get_object_or_404(ScenarioModel, id=scenario_id)
    option = get_object_or_404(LoanOptionModel, id=option_id, scenario=scenario)
    if request.method == "POST":
        option.delete()
        return redirect("web:scenario_compare", scenario_id=scenario.id)
    return render(request, "option_confirm_delete.html", {
        "scenario": scenario,
        "option": option,
    })
...
def rate_watch(request: HttpRequest) -> HttpResponse:
    latest_30 = BenchmarkPointModel.objects.filter(series="MORTGAGE30US").order_by("-observed_on").first()
    latest_15 = BenchmarkPointModel.objects.filter(series="MORTGAGE15US").order_by("-observed_on").first()

    points_30 = list(BenchmarkPointModel.objects.filter(series="MORTGAGE30US").order_by("observed_on")[:52])
    points_15 = list(BenchmarkPointModel.objects.filter(series="MORTGAGE15US").order_by("observed_on")[:52])
