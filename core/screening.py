...
def back_end_dti(
    scenario: ScenarioInput, proposed_housing_payment: Decimal
) -> Decimal | None:
    """Derive back-end Debt-to-Income (DTI) ratio from gross income, debt, and proposed housing payment."""
    if not scenario.gross_monthly_income or scenario.gross_monthly_income <= Decimal(
        "0"
    ):
        return None
...
def screen_conventional(
    ...
    if scenario_dti is None:
        return ScreeningResult(
            ScreeningState.MORE_INFO_NEEDED, "INCOME_OR_DEBT_MISSING"
        )
